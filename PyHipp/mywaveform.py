...
class Waveform(DPT.DPObject):
    ...
    def plot(self, i = None, ax = None, getNumEvents = False, getLevels = False,\
        ...
        plotOpts = {'PlotType': DPT.objects.ExclusiveOptions(['Channel', 'Array'], 0), \
            'LabelsOff': True, 'TitleOff': True, 'TicksOff': True}
        ...
        if getNumEvents:  
            ...
            if self.current_plot_type == plot_type:
                ...
                if(i == 0):
                    channel_num = 0
                    return self.numSets, channel_num
                for index, key in enumerate(self.array_dict):
                    if(index == i - 1):
                        return self.numSets, self.array_dict[key] + 1
        ...
        if plot_type == 'Channel':
            ...
            for index, key in enumerate(self.array_dict):
                if(index == i - 1):
                    cstart = self.array_dict[key] + 1
                if(index == i):
                    cend = self.array_dict[key]
    ...
    def plot_data(self, i, ax, plotOpts, isCorner):
        ...
        #if want to remove title can toggle it in the PanGUI window
